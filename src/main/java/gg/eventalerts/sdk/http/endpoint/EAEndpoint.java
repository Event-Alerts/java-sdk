    ...
    public APIResponse retrieve(@Nullable Map<String, Object> queryParams, @Nullable String... pathSegments) {
        ...
        try {
            ...
            if (json.has(endpointPath)) {
                result = GSONProvider.GSON.fromJson(json, TypeToken.getParameterized(PaginatedResponse.class, getObjectClass()).getType());

                // Inject objects into response.data
                ((PaginatedResponse<O>) result).data = GSONProvider.GSON.fromJson(json.get(endpointPath), TypeToken.getParameterized(List.class, getObjectClass()).getType());
            ...
            // Single
            result = GSONProvider.GSON.fromJson(json, TypeToken.getParameterized(SingleResponse.class, getObjectClass()).getType());
